...
//unsigned char in1[32] = {41, 39, 14, 10, 49, 62, 37, 1, 17, 22, 9, 38, 28, 33, 8, 26, 46, 23, 64, 13, 54, 55, 27, 12, 56, 36,  3, 4, 15, 32, 19, 45};
//unsigned char in2[32] = {21, 48, 34, 63, 52, 53, 16, 11, 44, 59, 43, 20, 57, 60, 30, 24, 29, 7, 47, 18, 6, 58, 25, 50, 31, 61, 51, 35, 40, 2, 5, 42};
...
int main(unsigned hart_id) {
    ...
    if (hart_id == 0) {
        ...
        write_data_to_ACC(PE_START_ADDR_0, in + 32, 16);
        write_data_to_ACC(PE_START_ADDR_1, in + 48, 16);
        sem_post(&DMA_flag[1]);

        sem_wait(&DMA_flag[0]);
        read_data_from_ACC(PE_READ_ADDR_0, tmp0, 16);
        read_data_from_ACC(PE_READ_ADDR_1, tmp0 + 16, 16);
        read_data_from_ACC(PE_READ_ADDR_0, tmp0 + 32, 16);
        read_data_from_ACC(PE_READ_ADDR_1, tmp0 + 48, 16);
        sem_post(&DMA_flag[1]);
    ...
    if (hart_id == 1) {
        ...
        sem_wait(&DMA_flag[1]);
        write_data_to_ACC(PE_START_ADDR_2, in + 64, 16);
        write_data_to_ACC(PE_START_ADDR_3, in + 80, 16);
        write_data_to_ACC(PE_START_ADDR_2, in + 96, 16);
        write_data_to_ACC(PE_START_ADDR_3, in + 112, 16);
        sem_post(&DMA_flag[0]);

        sem_wait(&DMA_flag[1]);
        read_data_from_ACC(PE_READ_ADDR_2, tmp1, 16);
        read_data_from_ACC(PE_READ_ADDR_3, tmp1 + 16, 16);
        read_data_from_ACC(PE_READ_ADDR_2, tmp1 + 32, 16);
        read_data_from_ACC(PE_READ_ADDR_3, tmp1 + 48, 16);
        sem_post(&DMA_flag[0]);
    ...
    if (hart_id == 0) {
        int W = 128;
        for (int i = 1; i <= 128 / W; i++) {
            int l1 = 0, l2 = 0;
            printf("l1: %d, l2: %d\n", l1, l2);
            for (int index = l1; index < i * W; index++) {
                if (l1 < 64 && (out0[l1] < out1[l2]) || l2 >= 64) {
